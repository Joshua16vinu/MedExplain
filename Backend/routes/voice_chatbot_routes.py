...
from services.voice_service import (
    ...
    generate_voice_chat_response  # ⭐ Use voice-optimized response generator
)
from services.chatbot_repository import (
    get_chat_session,
    add_message_to_session
...
def send_voice_message():
    ...
    try:
        ...
        print(f"\n{'='*60}")
        print(f"🎤 Voice Message Request")
        print(f"Session ID: {session_id}")
        print(f"Language: {language}, Gender: {gender}")
        print(f"Audio size: {len(audio_base64) if audio_base64 else 0} chars")
        print(f"User ID: {g.user.get('uid')}")
        print(f"{'='*60}")
        ...
        print(f"✅ Session found: {session.get('reportName')}")
        
        # Import get_report_with_retry to get the report
        from routes.chatbot_routes import get_report_with_retry
        ...
        if not report:
            print(f"❌ Report not found: {session.get('reportName')}")
            return {"error": "Report not found. Please try uploading it again."}, 404
        ...
        # Step 5: Generate AI response using VOICE-OPTIMIZED function
        print("Step 5: Generating SHORT voice response...")
        bot_response = generate_voice_chat_response(
            user_message,
            report_summary,
            messages,
            language
        ...
        print(f"✅ Bot response ({len(bot_response)} chars): '{bot_response[:100]}...'")
        
        # Step 6: Add bot response to session
        add_message_to_session(session_id, "assistant", bot_response, g.user["uid"])
        
        # Step 7: Convert response to speech
        print("Step 7: Converting response to speech...")
        response_audio_bytes = text_to_speech(bot_response, language, gender)
        response_audio_base64 = base64.b64encode(response_audio_bytes).decode('utf-8')
